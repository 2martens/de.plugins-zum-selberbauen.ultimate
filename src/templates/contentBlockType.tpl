	...
	{foreach from=$contents key=contentID item=content}
		{if $content->status == 3}
			...
			{/if}
				...
				<article itemtype="http://schema.org/Article" itemscope="itemscope" class="{if $requestType != 'page'}ultimateContent dividers{else}container containerPadding marginTop{/if}">
					
					{if !$block->hideContent}
						{if $requestType != 'page'}
						<div class="message jsMessage messageReduced"
				...
				data-user-id="{@$content->authorID}">
						...
						<header class="{if $requestType != 'page'}messageHeader{else}boxHeadline{/if}">
							{if $requestType != 'page'}
									...
									<footer class="messageOptions">
										<nav class="jsMobileNavigation buttonGroupNavigation">
											<ul class="smallButtons buttonGroup">{*
												*}{if $requestType != 'content'}<li><a href="{linkExtended application='ultimate' date=$content->publishDateObject->format('Y-m-d') contentSlug=$content->contentSlug}{/linkExtended}" class="button"><span class="icon icon16 icon-arrow-right"></span> <span>{lang}wcf.global.button.readMore{/lang}</span></a></li>{/if}{*
