	...
	<nav>
		<ul>
			{if $__wcf->session->getPermission('admin.content.ultimate.canManageTemplates')}
				<li><a href="{link application='ultimate' controller='UltimateTemplateAdd'}{/link}" title="{lang}wcf.acp.ultimate.template.add{/lang}" class="button"><span class="icon icon16 icon-plus"></span> <span>{lang}wcf.acp.ultimate.template.add{/lang}</span></a></li>
			{/if}
