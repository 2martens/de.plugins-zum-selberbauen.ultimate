 ...
 * @license http://www.gnu.org/licenses/lgpl-3.0 GNU Lesser General Public
 *          License, version 3
 */
...
ULTIMATE.ACP.Block.Transfer.prototype = {
    ...
    init : function(elementID, containerID, className) {
	    ...
	    $('#' + this._containerID).find('button[data-type="submit"]').click(
	            function(event) {
    ...
    _success : function(data, textStatus, jqXHR) {
	    ...
	    try {
		    var $data = data['returnValues'];
		    var $newHtml = '<li class="jsBlock sortableNode sortableNoNesting" data-object-name="'
		            + $data['blockTypeName'] + '" data-object-id="'
		            + $data['blockID'] + '">';
		    $newHtml += '<span class="sortableNodeLabel"><span class="statusDisplay sortableButtonContainer">';
 ...
 * @since version 1.0.0
 */
ULTIMATE.ACP.Menu.Item.Transfer = function(elementID, menuItemListID,
        className, offset, type) {
    ...
    _success : function(data, textStatus, jqXHR) {
	    ...
	    try {
		    ...
		    for ( var $menuItemID in data) {
			    ...
			    $newItemHtml += '</span></span><ol class="sortableList" data-object-id="'
			            + $menuItemID + '"></ol></li>';
