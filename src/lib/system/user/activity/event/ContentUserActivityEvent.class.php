<?php
/**
 * Contains the ContentUserActivityEvent class.
 * 
 * LICENSE:
...
/**
 * User activity event implementation for contents.
 * 
 * @author		Jim Martens
 * @copyright	2011-2013 Jim Martens
 * @license		http://www.gnu.org/licenses/lgpl-3.0 GNU Lesser General Public License, version 3
	...
	public function prepare(array $events) {
		...
		foreach ($events as $event) {
			if (isset($contents[$event->objectID])) {
				...
				// short output
				$text = WCF::getLanguage()->getDynamicVariable('wcf.user.profile.recentActivity.content', array(
					'content' => $content
				));
				$event->setTitle($text);
