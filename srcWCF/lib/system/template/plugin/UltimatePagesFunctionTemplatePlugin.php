<?php
/**
 * Contains the UltimatePagesFunctionTemplatePlugin class.
 *
 * LICENSE:
...
/**
 * Extends the WCF Pages function template plugin with Ultimate CMS specific needs.
 *
 * Usage (the same options as the WCF version):
 * * {ultimatePages controller='TestPage' application='ultimate' parent='EditSuite' link='page-%d.html'}
 * * {ultimatePages controller='TestPage' application='ultimate' parent='EditSuite' link='page-%d.html' print=true}
 * * {ultimatePages page=8 controller='TestPage' application='ultimate' link='page-%d.html' print=true}
 * * {ultimatePages page=8 pages=10 controller='TestPage' application='ultimate' link='page-%d.html' print=true}
 * * {ultimatePages pages=10 controller='TestPage' application='ultimate' link='page-%d.html'}
 * 
 * Of course the options can be combined with each other in other combinations. Mandatory is the link and controller.
 * The pages option is mandatory if that information is not already available as a template variable. If no application+
 * is given, the WCF is assumed.
 *
 * @author		Jim Martens
 * @copyright	2011-2014 Jim Martens
 * @license		http://www.gnu.org/licenses/lgpl-3.0 GNU Lesser General Public License, version 3
 ...
 * @category	Ultimate CMS
 */
class UltimatePagesFunctionTemplatePlugin extends PagesFunctionTemplatePlugin
{
	/**
	 * the controller
	 * @var string
	...
	protected function makeLink($link, $pageNo, $activePage, $pages) {
		...
		if ($activePage != $pageNo) {
			return '<li class="button"><a href="'.$this->insertPageNumber($link, $pageNo).'" title="'.
				WCF::getLanguage()->getDynamicVariable('wcf.page.pageNo', array('pageNo' => $pageNo)).'">'.
				StringUtil::formatInteger($pageNo).'</a></li>'."\n";
		}
		else {
			return '<li class="button active"><span>'.StringUtil::formatInteger($pageNo).
					'</span><span class="invisible">'.
					WCF::getLanguage()->getDynamicVariable('wcf.page.pagePosition', 
														array('pageNo' => $pageNo, 'pages' => $pages)).
					'</span></li>'."\n";
		}
	}
	...
	protected function makePreviousLink($link, $pageNo) {
		if ($pageNo > 1) {
			return '<li class="button skip"><a data-controller="'.
					$this->controller.'Page" data-request-type="page" href="'.
					$this->insertPageNumber($link, $pageNo - 1).'" title="'.
					WCF::getLanguage()->getDynamicVariable('wcf.global.page.previous').
					'" class="jsTooltip"><span class="icon icon16 icon-double-angle-left"></span></a></li>'."\n";
		}
		else {
	...
	protected function makeNextLink($link, $pageNo, $pages) {
		if ($pageNo && $pageNo < $pages) {
			return '<li class="button skip"><a data-controller="'.$this->controller.'Page" data-request-type="page" href="'.
					$this->insertPageNumber($link, $pageNo + 1).'" title="'.
					WCF::getLanguage()->getDynamicVariable('wcf.global.page.next').
					'" class="jsTooltip"><span class="icon icon16 icon-double-angle-right"></span></a></li>'."\n";
		}
		else {
	...
	public function execute($tagArgs, TemplateEngine $tplObj) {
		...
		$this->controller = $tagArgs['controller'];
		...
		if ($tagArgs['pages'] > 1) {
			...
			if (isset($tagArgs['parent'])) $parameters['parent'] = $tagArgs['parent'];
			...
			if ($left > 1) {
				if ($left - 1 < 2) {
					...
					$html .= '<li class="button jumpTo"><a title="'.
							WCF::getLanguage()->getDynamicVariable('wcf.global.page.jumpTo').'" class="jsTooltip">'.
							StringUtil::HELLIP.'</a></li>'."\n";
				}
			}
